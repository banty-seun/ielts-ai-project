...
async function run() {
  const rows = (
    await pool.query("select id from task_progress where lower(skill)='listening' order by created_at desc limit 5")
  ).rows;
  ...
  console.log("[ReleaseProbe][Readiness]", JSON.stringify(report));
